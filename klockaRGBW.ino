...
Senast ändrad: 2023-08-13 22:45
*/
#include <Arduino.h>
...
void setup() {
  ...
  //rtc.adjust(DateTime(0,0,0,13,39,0)); //Används för att testa custom tid
...
void ChangeTime(int _hour, int _minute) {
  ...
  if (LedsOFF_noshow() == true) {
    ...
    if (_minute >= 10 && _minute < 15) {
      minuteRounded = 10;
      minuteArrayNR = 9;
    }  // XX:10
    if (_minute >= 15 && _minute < 20) {
      minuteRounded = 15;
      minuteArrayNR = 10;
    }  //XX:15
    if (_minute >= 20 && _minute < 25) {
      minuteRounded = 20;
      minuteArrayNR = 11;
    }  //XX:20
    if (_minute >= 25 && _minute < 30) {
      minuteRounded = 25;
      minuteArrayNR = 5;
    }  // XX:25
    if (_minute >= 30 && _minute < 35) {
      minuteRounded = 30;
      minuteArrayNR = 6;
    }  // XX:30
    if (_minute >= 35 && _minute < 40) {
      minuteRounded = 35;
      minuteArrayNR = 7;
    }  // XX:35
    if (_minute >= 40 && _minute < 45) {
      minuteRounded = 40;
      minuteArrayNR = 4;
    }  // XX:40
    if (_minute >= 45 && _minute < 50) {
      minuteRounded = 45;
      minuteArrayNR = 3;
    }  // XX:45
    if (_minute >= 50 && _minute < 55) {
      minuteRounded = 50;
      minuteArrayNR = 2;
    }  // XX:50
    if (_minute >= 55) {
    ...
    if (_minute > 0 && _minute < 5) {
      ...
      strip.setPixelColor(150, 0, 0, 0, 0);
      strip.setPixelColor(151, 0, 0, 0, 0);
...
void btnLongPressStart() {
  ...
  if (brightnessUpDown == true) {
    brightnessUpDown = false;
  } else if (brightnessUpDown == false) {
    brightnessUpDown = true;
  }
